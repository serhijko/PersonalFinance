    ...
    public void postEdit(SaveData sd) {
        ...
        for (Currency c : sd.getCurrencies()) {
            for (Account account : sd.getAccounts()) {
                if (account.getCurrency().equals(c)) account.setCurrency(c);
            }
            for (Transaction transaction : sd.getTransactions())
                if (transaction.getAccount().getCurrency().equals(c)) transaction.getAccount().setCurrency(c);
            for (Transfer transfer : sd.getTransfers()) {
                if (transfer.getFromAccount().getCurrency().equals(c)) transfer.getFromAccount().setCurrency(c);
                if (transfer.getToAccount().getCurrency().equals(c)) transfer.getToAccount().setCurrency(c);
            }
        }
